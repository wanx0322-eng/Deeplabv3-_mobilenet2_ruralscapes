...
if __name__ == "__main__":
    ...
    seed            = 11
    #----------------------------------------------------------------------#
    #   deterministic   True  = 可复现（cudnn.benchmark 关闭，稍慢）
    #                   False = 允许 cudnn 自动寻优，更快但结果不可完全复现
    #----------------------------------------------------------------------#
    deterministic   = True
    #---------------------------------------------------------------------#
    ...
    if Cuda:
        if distributed:
            ...
            #   注意：cudnn.benchmark = True 会覆盖 seed_everything 里设的
            #   deterministic=True / benchmark=False，让随机种子形同虚设。
            #   只有在不要求可复现时才打开它换取速度。
    ...
    if True:
        ...
        #   drop_last=False，所以验证批数向上取整，才能覆盖全部验证图
